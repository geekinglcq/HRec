...
# Created Time: 2020-12-31 17:32:58
# ###########################
r"""
################################################
Reference:
...
from .layers import MLPLayers, RegLoss
...
class DCN(ContextModel):
    ...
    def __init__(self, config, dataset):
        ...
        self.cross_layer_parameter = [
            nn.Parameter(
                torch.empty(self.embedding_size * len(self.token_field_names) +
        ...
        in_feature_num = self.mlp_hidden_size[-1] + \
            self.embedding_size * len(self.token_field_names) + \
            len(self.float_field_names)
